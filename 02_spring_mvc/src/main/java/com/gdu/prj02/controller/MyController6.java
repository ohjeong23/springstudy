...
public class MyController6 {
  ...
  public String login1(HttpServletRequest request) {
    ...
    // 메인 페이지로 이동
    //return "index"; forward 를 선택
    //return "redirect:/main.do"; redirect 를 선택  이걸 권장
 ...
 // HttpSession 구하기
    ...
    //session 의 모든 정보 지우기 다 모두 다 지우기 모든 정보 날리는 메소드
    session.invalidate();
 ...
 // 메인 페이지로 이동
  ...
  public String login2(Model model) {
    ...
    // model 에 객체 저장하기(@SessionAttributes 에 의해서 session에도 저장된다.)
    model.addAttribute("user",user); // 모델에 했지만 세션에도 같이 저장됨
