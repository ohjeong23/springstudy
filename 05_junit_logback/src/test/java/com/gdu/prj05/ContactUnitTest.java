...
@FixMethodOrder(MethodSorters.NAME_ASCENDING)//이름의 정렬 순, 순서 정리
...
//dao unitest 만들어서 하는 테스트 
  ...
  public void test02_조회() {
    ...
    assertNotNull(contact); //널이 아니면 조회가 됨 성공
  ...
  public void test03_수정() {
    ...
    ContactDto contact = ContactDto.builder()
        ...
        .email("[수정]테스트이메일")
        .address("[수정]테스트주소")
        .contactNo(1) // 어떤 절에서 수정했는지 알기위해서 넘버가 필요
      .build();
  ...
  public void test04_목록() {
    ...
    assertEquals(1, contactList.size()); //컨텍리스트에 들어있는 데이터 개수가 1개이길 
  ...
  public void test05_삭제() {
    ...
    assertEquals(1, deleteCount);//한개가 삭제되길 
